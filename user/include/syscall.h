...
struct syscall_frame {
    SyscallNum num;
    unsigned argc;
    unsigned argv[5]; // Max five arguments
};
...
#define VA_NARGS_IMPL(_0, _1, _2, _3, _4, _5, N, ...) N
...
#define SYSCALL_BUILD_FRAME(num, argc, a0, a1, a2, a3, a4)                     \
    syscall_frame {                                                            \
        (num), (argc), { (a0), (a1), (a2), (a3), (a4) }                        \
    }

// Helper layer to force argument expansion order
#define _SYSCALL_EXPAND(num, argc, a0, a1, a2, a3, a4, ...)                    \
    SYSCALL_BUILD_FRAME(num, argc, a0, a1, a2, a3, a4)

// Public syscall macro
#define syscall(num, ...)                                                      \
    ({                                                                         \
        syscall_frame f = _SYSCALL_EXPAND(num, VA_NARGS(__VA_ARGS__),          \
                                          ##__VA_ARGS__, 0, 0, 0, 0, 0);       \
        syscall_raw(&f);                                                       \
    })
